        ...
        <div class="flex flex-row mt-3 md:flex-row md:space-x-4 space-y-4 md:space-y-0">
            <div class="flex-2 m-2">
                ...
                <input wire:model="description" type="text" id="description" value="{{ $transaction->description }}"
                    class="mt-1 p-1 block bg-gray-100 md:w-md rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                @error('description') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            <div class="flex-1 m-2">
                <label for="amount" class="block text-sm font-medium text-gray-700">Amount</label>
                <input wire:model="amount" type="number" step="0.01" id="amount" value="{{ $transaction->amount }}"
                    class="mt-1 p-1 block bg-gray-100 rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
        ...
        <div class="flex flex-row mt-3 md:flex-row md:space-x-4 space-y-4 md:space-y-0">
            <div class="flex-1">
                ...
                <select wire:model="category_id" id="category_id"
                    class="mt-1 p-1 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
